...
def main():
    ...
    for file_path in folder_path.glob('train/*.csv'):
        ...
        try:
            ...
            kan =KAN(width=[X_train.size()[1],3,1], grid=4, k=3, seed=seed, device=device, auto_save=False)

            t0 = time.perf_counter()
            kan.fit(dataset, opt="LBFGS", steps=100, lamb=0.001, earlyStop=True);
            ...
            results.append([function_name, None, R2_score_kan, t_lbfgs, None, None])
